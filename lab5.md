...
``` r
met <- data.table::fread("C:/Users/yemin/Desktop/PM566/week3/met_all.gz")
stations <- data.table::fread("C:/Users/yemin/Desktop/PM566/week5/isd-history.csv")
```
...
#   ]
#met_stations[,record2 := rank(abs(temp-temp50s)),by=STATE][record2==1,.(USAFID, temp, temp50s, STATE)] 
#also OK to use rank
# ATM press
...
``` r
met_stations3 <- unique(met[, .(USAFID, STATE, lon, lat)])

met_stations3[, n := 1:.N, by = USAFID]
met_stations3 <- met_stations3[n == 1]
# This is a short cut using the .SD keyword
# met_stations[, .SD[1], by = USAFID]  Select the first line of each group, and transfer met_stations to it without using met_stations <- .

met_stations3[, lat_mid := quantile(lat, probs = .5, na.rm = TRUE), by = STATE]
met_stations3[, lon_mid := quantile(lon, probs = .5, na.rm = TRUE), by = STATE]

# Looking at the euclidean distances
met_stations3[,  distance := sqrt((lat - lat_mid)^2 + (lon - lon_mid)^2)]
met_stations3[, minrecord := which.min(distance), by = STATE]
met_stations3[, n := 1:.N, by = STATE]
met_location <- met_stations3[n == minrecord, .(USAFID, STATE, lon, lat)]
met_location
```
